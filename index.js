...
// Fetch orders with populated product details and map through them
app.get('/api/orders', async (req, res) => {
    try {
        const orders = await Order.find().populate('products.pid', 'name price');

        const ordersWithDetails = orders.map(order => ({
            ...order.toObject(),
            products: order.products.map(p => ({
                name: p.pid ? p.pid.name : 'Unknown Product', // Fallback for missing product data
                price: p.pid ? p.pid.price : 0,
                count: p.count
            }))
        }));
...
// Route to fetch details of a specific order by ID
app.get('/api/orders/:id', async (req, res) => {
    ...
    try {
        ...
        // Ensure products are populated and handle missing values
        const orderWithDetails = {
            ...order.toObject(),
            products: order.products.map(p => ({
                name: p.pid ? p.pid.name : 'Unknown Product',
                price: p.pid ? p.pid.price : 0,
                count: p.count
            }))
        };
...
app.patch('/api/orders/:id', async (req, res) => {
    ...
    if (!status) {
        return res.status(400).send({ error: 'Status is required to update order' });
    }
    ...
    try {
        ...
        if (result.nModified === 0) {
            return res.status(404).send({ error: 'Order not found or already updated' });
        }
